...
module.exports = class Functions {
    ...
    async updateTicketHistory(ticket) {
        try {
            ...
            let newMessageEmbed = new EmbedBuilder()
    ...
    async getFormattedTime(time) {
        try {
            ...
            if (days > 0) { time = time - (days * (60 * 60 * 24 * 1000)); if (days == 1) { formatted.push(`${days} d`) } else { formatted.push(`${days}ds`) }; };
            let hours = Math.floor(time / (60 * 60 * 1000));
            if (hours > 0) { time = time - (hours * (60 * 60 * 1000)); if (hours == 1) { formatted.push(`${hours}hr`) } else { formatted.push(`${hours}hrs`) }; };
            let minutes = Math.floor(time / (60 * 1000));
            if (minutes > 0) { time = time - (minutes * (60 * 1000)); if (minutes == 1) { formatted.push(`${minutes}m`) } else { formatted.push(`${minutes}m`) }; };
            let seconds = Math.floor(time / 1000);
            if (seconds > 0) { time = time - (seconds * (60 * 1000)); if (seconds == 1) { formatted.push(`${seconds}s`) } else { formatted.push(`${seconds}s`) }; };
